...
int main()
{
	/*{
		...
		s1.getData();
		cout << "\n-----------------------------------------------------------------------\n";
		cout << setw(5) << left <<  "R No" << setw(20) << left << "Name" << setw(6) << left << "Class" << setw(11) << left << "Birth Date"  << setw(7) << left << "BGroup" << setw(25) << left << "Address" << setw(11) << left <<  "Mobile No" << setw(4) << left << "S1" << setw(4) << left  << "S2" << setw(4) << left << "S3" << setw(6) << left << "Total"  << setw(8) << left << "Average" << setw(20) << left << "Grade" << endl;
		s1.putData();
		Student s2(s1);
		s2.putData();
		cout << "\n------------------------------------------------------------------------\n";
	...
	do
	{
		...
		switch(choice)
		{
			...
			case 3 : cout << "Enter Roll Number to Search Record: ";
				...
				break;
			case 4 : cout << "Data Entry was Done g+out of" << Student ::getStudentCount() << "/" << cnt << endl;
				break;
			case 5 : cout << "Enter Roll Number to Search Record:";
				cin >> key;
				for(i=0,flag=0;i<Student::getStudentCount();i++)
				{
					if(key == st[i].getRno())
